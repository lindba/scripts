...
ntns(){ ip link add $nsh type veth peer name $nsc address $nscMac;    #veth
 ...
 ip netns exec $ns ip addr add $ip/$pfx dev $nsc; ip netns exec $ns ip route add default via $gw dev $nsc; ip netns; }

mnt(){ mkdir -p $nsm; mount --make-rprivate /; mount --make-shared -o bind $nsr $nsm;
 mkdir -p $nsm/proc $nsm/sys $nsm/sys/kernel/debug $nsm/dev $nsm/dev/pts  $nsm/var; mount --rbind /dev $nsm/dev/; mount --rbind /dev/pts $nsm/dev/pts;
...
pvr(){ ntns; [[ $act != puip ]] && mnt; ipt; cd $nsm;
...
<<tmp
 ip netns exec $ns unshare --mount --uts --ipc --mount-proc=/proc --pid --fork /bin/bash - <<pvr
  ...
  /usr/sbin/sshd -D -o PidFile=/run/sshd-$ns.pid -E /tmp/sshd.log -o ListenAddress=$ip;  #!systemd
pvr
tmp
}
